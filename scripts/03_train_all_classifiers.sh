...
for dataset_name in "age" "age_short" "age_tinkoff" "rosbank"; do
    ...
    for clf_type in "target" "substitute"; do
        ...
        for config_name in "gru_with_amounts" "lstm_with_amounts" "cnn_with_amounts"; do
            # printf "./scripts/local/train_clf.sh ${config_name} ${clf_type} "100_quantile" ${dataset_name}\n"
 ...
 cp -r ./experiments/trained_models/rosbank/substitute_clf/lstm_with_amounts/model.tar.gz ./presets/rosbank/models/substitute_clf/lstm_with_amounts.tar.gz
